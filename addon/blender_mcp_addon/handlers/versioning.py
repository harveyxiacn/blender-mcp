...
def _load_manifest() -> dict:
    ...
    return {
        ...
        "current_branch": "main",
        "branches": {"main": {"versions": [], "head": None}},
    }
...
def handle_init(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if project_path and not bpy.data.filepath:
        ...
        return {"success": False, "error": {"code": "INIT_ERROR", "message": str(e)}}
...
def handle_restore(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": {"code": "RESTORE_ERROR", "message": str(e)}}
    ...
    try:
        ...
        diff = {
            ...
            "changes": {
                ...
                "materials_diff": v2_info.get("materials_count", 0)
                - v1_info.get("materials_count", 0),
            },
        }
        ...
        return {"success": False, "error": {"code": "COMPARE_ERROR", "message": str(e)}}
...
def handle_branch(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": False, "error": {"code": "BRANCH_ERROR", "message": str(e)}}
...
def handle_branches(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {
            "success": True,
            "data": {"current_branch": manifest["current_branch"], "branches": branches},
        }

    except Exception as e:
        return {"success": False, "error": {"code": "BRANCHES_ERROR", "message": str(e)}}
...
def handle_switch_branch(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "data": {"branch": branch_name, "head": head}}

    except Exception as e:
        return {"success": False, "error": {"code": "SWITCH_ERROR", "message": str(e)}}
    ...
    try:
        ...
        if manifest["branches"][branch]["head"] == version_id and not force:
            return {
                ...
                "error": {
                    ...
                    "message": "Cannot delete current head version, use force=True to force delete",
                },
            }
        ...
        if manifest["branches"][branch]["head"] == version_id:
            if manifest["branches"][branch]["versions"]:
                manifest["branches"][branch]["head"] = manifest["branches"][branch]["versions"][-1][
                    "id"
                ]
            else:
        ...
        return {"success": True, "data": {"deleted_version": version_id}}
...
def handle_info(params: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for v in manifest["branches"][branch]["versions"]:
            if v["id"] == version_id:
                return {"success": True, "data": {"branch": branch, "version": v}}

...
class EdgeMarkInput(BaseModel):
    ...
    object_name: str = Field(..., description="Object name")
    mark_type: EdgeMarkOperation = Field(
        ..., description="Mark type: CREASE, SHARP, SEAM (UV seam), BEVEL_WEIGHT"
    )
    value: float = Field(
        default=1.0, description="Mark value (0.0-1.0), 0=clear mark", ge=0.0, le=1.0
    )
    clear: bool = Field(
        default=False,
        description="Whether to clear marks (set to true to clear marks on selected edges)",
    )
...
class VertexGroupInput(BaseModel):
    ...
    vertex_indices: list[int] | None = Field(
        default=None, description="Vertex index list (uses current selection if empty)"
    )
...
class VertexColorInput(BaseModel):
    ...
    action: str = Field(
        default="CREATE",
        description="Action: CREATE (create layer), PAINT (paint), FILL (fill all)",
    )
    layer_name: str = Field(default="Col", description="Vertex color layer name")
    color: list[float] | None = Field(default=None, description="Color [R,G,B,A] (0-1 range)")
    face_indices: list[int] | None = Field(
        default=None, description="Face index list (PAINT only, applies to all faces if empty)"
    )
    ...
    async def blender_mesh_edit_advanced(params: MeshEditAdvancedInput) -> str:
        ...
        if result.get("success"):
            op_names = {
                ...
                "BRIDGE_EDGE_LOOPS": "Bridge Edge Loops",
                "SPIN": "Spin",
                "KNIFE_CUT": "Knife Cut",
                "FILL": "Fill",
                "GRID_FILL": "Grid Fill",
                "SEPARATE": "Separate",
                "SYMMETRIZE": "Symmetrize",
                "POKE_FACES": "Poke Faces",
                "TRIANGULATE": "Triangulate",
                "TRIS_TO_QUADS": "Tris to Quads",
                "DISSOLVE": "Dissolve",
            }
            return (
                f"{op_names.get(params.operation.value, params.operation.value)} operation complete"
            )
        else:

...
# === 头部 (方脸, 更男性化) ===
bpy.ops.mesh.primitive_uv_sphere_add(segments=28, ring_count=18, radius=0.125, location=(kx, 0, 1.66))
head = bpy.context.active_object; head.name='KL_Head'
...
for side, sx in [('L', 1), ('R', -1)]:
    bpy.ops.mesh.primitive_uv_sphere_add(segments=12, ring_count=8, radius=0.022,
        location=(kx+sx*0.045, -0.095, 1.665))
    ew = bpy.context.active_object; ew.name=f'KL_EyeW{side}'
    ew.scale=(1.4, 0.5, 0.7); bpy.ops.object.transform_apply(scale=True)
    setup(ew, 'M_EyeWhite', ss=1, outline=0)

    bpy.ops.mesh.primitive_uv_sphere_add(segments=12, ring_count=8, radius=0.016,
        location=(kx+sx*0.045, -0.108, 1.663))
    iris = bpy.context.active_object; iris.name=f'KL_Iris{side}'
    iris.scale=(1.3, 0.5, 0.7); bpy.ops.object.transform_apply(scale=True)
    setup(iris, 'M_KleinBrown', ss=1, outline=0)

    bpy.ops.mesh.primitive_uv_sphere_add(segments=8, ring_count=6, radius=0.008,
        location=(kx+sx*0.045, -0.115, 1.662))
    pupil = bpy.context.active_object; pupil.name=f'KL_Pupil{side}'
    pupil.scale=(1.0, 0.5, 0.8); bpy.ops.object.transform_apply(scale=True)
    setup(pupil, 'M_EyeBlack', ss=0, outline=0)

# 眉毛 (粗)
for side, sx in [('L', 1), ('R', -1)]:
    bpy.ops.mesh.primitive_cube_add(size=0.010, location=(kx+sx*0.045, -0.110, 1.695))
    brow = bpy.context.active_object; brow.name=f'KL_Brow{side}'
...
bpy.ops.mesh.primitive_uv_sphere_add(segments=8, ring_count=6, radius=0.010,
    location=(kx, -0.118, 1.635))
nose = bpy.context.active_object; nose.name='KL_Nose'
...
mouth = bpy.context.active_object; mouth.name='KL_Mouth'
mouth.scale=(2.5, 0.3, 0.2); bpy.ops.object.transform_apply(scale=True)
...
for side, mx in [('L', 1), ('R', -1)]:
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(segments=10, ring_count=8, radius=0.030,
        location=(kx+mx*0.36, 0, 0.74))
    hand = bpy.context.active_object; hand.name=f'KL_Hand{side}'
    hand.scale=(0.9, 0.6, 1.2); bpy.ops.object.transform_apply(scale=True)
    setup(hand, 'M_SkinTan', ss=1, outline=0)

# 袴裤腿 (参考图: 奶色宽松袴裤, 脚踝收束)
for side, mx in [('L', 1), ('R', -1)]:
    hakama = loft(f'KL_Hakama{side}', [
...
for side, mx in [('L', 1), ('R', -1)]:
    ...
    # 绑带
    for zi in range(3):
        bpy.ops.mesh.primitive_torus_add(major_radius=0.042, minor_radius=0.003,
            location=(kx+mx*0.085, 0, 0.06 + zi*0.035))
        strap = bpy.context.active_object; strap.name=f'KL_SandalStrap{side}{zi}'
...
# === 头发 (参考图: 竖直向上的大束红色刺发) ===
bpy.ops.mesh.primitive_uv_sphere_add(segments=20, ring_count=14, radius=0.135,
    location=(kx, 0.01, 1.72))
hb = bpy.context.active_object; hb.name='KL_HairBase'
...
# 红色头带 (参考图: 红底+白菱形花纹)
bpy.ops.mesh.primitive_torus_add(major_radius=0.140, minor_radius=0.012,
    location=(kx, 0, 1.74))
hband = bpy.context.active_object; hband.name='KL_Headband'
...
# 头带菱形花纹 (白色)
for xi in range(-2, 3):
    bpy.ops.mesh.primitive_cube_add(size=0.010,
        location=(kx + xi*0.025, -0.130, 1.74))
    diamond = bpy.context.active_object; diamond.name=f'KL_Diamond{xi+2}'
...
# === 武士装 ===
# 深棕色胸甲 (参考图: 覆盖上胸的暗色胸甲)
chest_armor = loft('KL_ChestArmor', [
...
kb = bpy.context.active_object; kb.name='KL_BeltBuckle'
kb.scale=(1.2, 0.5, 0.8); bpy.ops.object.transform_apply(scale=True)
...
bpy.ops.mesh.primitive_cylinder_add(vertices=16, radius=0.022, depth=0.005,
    location=(ksx, -0.08, 0.54))
tsuba = bpy.context.active_object; tsuba.name='KT_Tsuba'
setup(tsuba, 'M_MetalDark', col_name='Weapons', ss=1, outline=0.002)

kt_grip = loft('KT_Grip', [
...
for i in range(6):
    z = 0.40 + i*0.022
    bpy.ops.mesh.primitive_torus_add(major_radius=0.012, minor_radius=0.002,
        location=(ksx, -0.08, z))
    wr = bpy.context.active_object; wr.name=f'KT_Wrap{i}'
    wr.scale=(1.0, 1.0, 0.3); bpy.ops.object.transform_apply(scale=True)
    setup(wr, 'M_FabricWhite', col_name='Weapons', ss=0, outline=0)

bpy.ops.mesh.primitive_uv_sphere_add(segments=8, ring_count=6, radius=0.010,
    location=(ksx, -0.08, 0.38))
pm = bpy.context.active_object; pm.name='KT_Pommel'
pm.scale=(1.0, 1.0, 0.7); bpy.ops.object.transform_apply(scale=True)
setup(pm, 'M_MetalDark', col_name='Weapons', ss=1, outline=0)

# 刀鞘 (挂在腰带)
sheath = loft('KT_Sheath', [
...
for dx in [0.060, -0.060]:
    bpy.ops.mesh.primitive_uv_sphere_add(segments=10, ring_count=8, radius=0.008,
        location=(ex+dx, -0.10, 0.50))
    gb = bpy.context.active_object; gb.name=f'E_GuardBall_{("L" if dx>0 else "R")}'
...
for i in range(8):
    z = 0.395 + i*0.012
    bpy.ops.mesh.primitive_torus_add(major_radius=0.014, minor_radius=0.002,
        location=(ex, -0.10, z))
    wr = bpy.context.active_object; wr.name=f'E_GripWrap{i}'
    wr.scale=(1.0, 1.0, 0.25); bpy.ops.object.transform_apply(scale=True)
...
bpy.ops.mesh.primitive_uv_sphere_add(segments=10, ring_count=8, radius=0.013,
    location=(ex, -0.10, 0.38))
pommel = bpy.context.active_object; pommel.name='E_Pommel'
pommel.scale=(1.0, 1.0, 0.7); bpy.ops.object.transform_apply(scale=True)

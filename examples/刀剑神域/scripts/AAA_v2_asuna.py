...
# 鼻/嘴
bpy.ops.mesh.primitive_uv_sphere_add(
    segments=8, ring_count=6, radius=0.006, location=(ax, -0.108, 1.555)
)
nose = bpy.context.active_object
nose.name = "A_Nose"
nose.scale = (0.5, 0.5, 0.7)
bpy.ops.object.transform_apply(scale=True)
...
for side, mx in [("L", 1), ("R", -1)]:
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=10, ring_count=8, radius=0.022, location=(ax + mx * 0.28, 0, 0.72)
    )
    hand = bpy.context.active_object
    hand.name = f"A_Hand{side}"
    hand.scale = (0.85, 0.5, 1.2)
    bpy.ops.object.transform_apply(scale=True)
...
for side, sx in [("L", 1), ("R", -1)]:
    ...
    # 辫子末端蝴蝶结
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=8, ring_count=6, radius=0.012, location=(ax + sx * 0.07, 0.03, 0.77)
    )
    ribbon = bpy.context.active_object
    ribbon.name = f"A_Ribbon{side}"
    setup(ribbon, "M_AsunaRed", ss=0, outline=0)

# 发箍 (参考图: 白色头箍)
bpy.ops.mesh.primitive_torus_add(major_radius=0.122, minor_radius=0.007, location=(ax, 0, 1.67))
...
for side, mx in [("L", 1), ("R", -1)]:
    ...
    bpy.ops.mesh.primitive_torus_add(
        major_radius=0.038, minor_radius=0.003, location=(ax + mx * 0.16, -0.01, 1.28)
    )
    spt = bpy.context.active_object
    spt.name = f"A_ShoulderTrim{side}"
...
# 腰带扣
bpy.ops.mesh.primitive_uv_sphere_add(
    segments=10, ring_count=8, radius=0.010, location=(ax, -0.105, 0.84)
)
ab = bpy.context.active_object
ab.name = "A_BeltBuckle"
ab.scale = (1.2, 0.5, 1.0)
bpy.ops.object.transform_apply(scale=True)
...
bpy.ops.mesh.primitive_torus_add(major_radius=0.028, minor_radius=0.004, location=(lx, -0.08, 0.64))
...
bpy.ops.mesh.primitive_uv_sphere_add(
    segments=8, ring_count=6, radius=0.009, location=(lx, -0.08, 0.53)
)

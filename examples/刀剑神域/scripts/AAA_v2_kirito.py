...
def toon_mat(name, rgb, shadow=(0.55, 0.50, 0.60), rp=0.42, rim=0.3, spec=False):
    ...
    diff.inputs["Color"].default_value = (*rgb, 1)
    diff.location = (-400, 100)
    s2r = N.new("ShaderNodeShaderToRGB")
    s2r.location = (-200, 100)
    ...
    if spec:
        ...
        gl.location = (-400, -400)
        sr2 = N.new("ShaderNodeShaderToRGB")
        sr2.location = (-200, -400)
        L.new(gl.outputs["BSDF"], sr2.inputs["Shader"])
        sramp = N.new("ShaderNodeValToRGB")
        sramp.location = (0, -400)
        ...
        sa = N.new("ShaderNodeMixRGB")
        sa.blend_type = "ADD"
        sa.inputs["Fac"].default_value = 0.15
        sa.location = (600, 0)
...
def metal_mat(name, rgb):
    ...
    out.location = (800, 0)
    gl = N.new("ShaderNodeBsdfGlossy")
    gl.inputs["Color"].default_value = (*rgb, 1)
...
ns = om.node_tree.nodes
ls = om.node_tree.links
ns.clear()
o = ns.new("ShaderNodeOutputMaterial")
o.location = (300, 0)
e = ns.new("ShaderNodeEmission")
e.inputs["Color"].default_value = (0.01, 0.01, 0.02, 1)
e.inputs["Strength"].default_value = 0
e.location = (100, 0)
...
eh.use_nodes = True
ns = eh.node_tree.nodes
ls = eh.node_tree.links
ns.clear()
o = ns.new("ShaderNodeOutputMaterial")
o.location = (300, 0)
e = ns.new("ShaderNodeEmission")
...
for side, sx in [("L", 1), ("R", -1)]:
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=6, ring_count=4, radius=0.005, location=(sx * 0.035, -0.122, 1.612)
    )
    hl2 = bpy.context.active_object
    hl2.name = f"K_HL2{side}"
    setup(hl2, "M_EyeHighlight", ss=0, outline=0)

# 鼻子 (微小凸起)
bpy.ops.mesh.primitive_uv_sphere_add(
    segments=8, ring_count=6, radius=0.008, location=(0, -0.118, 1.595)
)
...
for side, mx in [("L", 1), ("R", -1)]:
    ...
    # 手 (带简化手指)
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=12, ring_count=8, radius=0.025, location=(mx * 0.32, 0, 0.76)
    )
    hand = bpy.context.active_object
    hand.name = f"K_Hand{side}"
...
for side, sx in [("L", 1), ("R", -1)]:
    ...
    bpy.ops.mesh.primitive_torus_add(
        major_radius=0.012, minor_radius=0.003, location=(0, -0.125, 1.10)
    )
    buckle = bpy.context.active_object
    buckle.name = f"K_StrapBuckle{side}"
...
for side, x in [("L", 0.17), ("R", -0.17)]:
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=10, ring_count=8, radius=0.010, location=(x, -0.06, 1.34)
    )
    pin = bpy.context.active_object
    pin.name = f"K_ShoulderPin{side}"

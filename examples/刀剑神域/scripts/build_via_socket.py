...
class BlenderSocket:
    ...
    def create(self, obj_type, name, loc=(0,0,0), rot=(0,0,0), scale=(1,1,1), mesh_params=None):
        return self.cmd("object", "create", {
            "type": obj_type, "name": name,
            "location": list(loc), "rotation": list(rot), "scale": list(scale),
            "mesh_params": mesh_params or {}
        })
    
    def transform(self, name, loc=None, rot=None, scale=None):
        p = {"name": name}
        if loc: p["location"] = list(loc)
        if rot: p["rotation"] = list(rot)
        if scale: p["scale"] = list(scale)
        return self.cmd("object", "transform", p)
    
    def mat_create(self, name, color, metallic=0.0, roughness=0.5):
        return self.cmd("material", "create", {
            "name": name, "color": list(color) + [1.0] if len(color)==3 else list(color),
            "metallic": metallic, "roughness": roughness
        })
    
    def mat_assign(self, obj_name, mat_name):
        return self.cmd("material", "assign", {
            "object_name": obj_name, "material_name": mat_name
        })
    
    def light(self, name, loc, rot, energy, color):
...
# 灯光
B.create("LIGHT", "SAO_KeyLight", loc=(5,-5,10), rot=(math.radians(45),math.radians(15),math.radians(30)))
B.create("LIGHT", "SAO_FillLight", loc=(-5,-3,5), rot=(math.radians(60),math.radians(-30),math.radians(-20)))
...
for side, x in [('L',0.45),('R',-0.45)]:
    B.create("CYLINDER", f"CHR_Kirito_Arm_{side}", loc=(x,0,1.1),
             mesh_params={"vertices":12,"radius":0.08,"depth":0.55})
    B.mat_assign(f"CHR_Kirito_Arm_{side}", "SAO_Kirito_Black")

# 手
for side, x in [('L',0.45),('R',-0.45)]:
    B.create("UV_SPHERE", f"CHR_Kirito_Hand_{side}", loc=(x,0,0.78),
             scale=(1.0,0.7,1.2), mesh_params={"segments":10,"ring_count":8,"radius":0.06})
    B.mat_assign(f"CHR_Kirito_Hand_{side}", "SAO_Skin")

# 腿
for side, x in [('L',0.13),('R',-0.13)]:
    B.create("CYLINDER", f"CHR_Kirito_Leg_{side}", loc=(x,0,0.33),
...
# 头发基础
B.create("UV_SPHERE", "CHR_Kirito_HairBase", loc=(0,0.02,1.76),
         scale=(1.05,1.0,1.0), mesh_params={"segments":16,"ring_count":12,"radius":0.25})
...
for i,(x,y,z,ln,w,rot) in enumerate(bangs):
    B.create("CONE", f"CHR_Kirito_Bang_{i}", loc=(x,y,z),
...
for mult in [1, -1]:
    ...
    for i,(x,y,z,ln,w,rz,rx) in enumerate(sides):
        B.create("CONE", f"CHR_Kirito_Side_{sn}_{i}", loc=(x*mult,y,z),
...
for i,(x,y,z,ln,w,rz) in enumerate(backs):
    B.create("CONE", f"CHR_Kirito_Back_{i}", loc=(x,y,z),
...
# 头发
B.create("UV_SPHERE", "CHR_Asuna_HairBase", loc=(ox,0.02,1.72),
...
# 刘海
a_bangs = [(-0.08,-0.18,1.78,0.12,0.035,12),(-0.02,-0.20,1.80,0.11,0.03,5),
           (0.04,-0.19,1.79,0.13,0.035,-3),(0.10,-0.17,1.77,0.11,0.03,-10)]
for i,(x,y,z,ln,w,rot) in enumerate(a_bangs):
    B.create("CONE", f"CHR_Asuna_Bang_{i}", loc=(ox+x,y,z),
             rot=(math.radians(70),0,math.radians(rot)),
             mesh_params={"vertices":8,"radius1":w,"radius2":0.005,"depth":ln})
...
for side, sx2 in [('L',0.15),('R',-0.15)]:
    B.create("CYLINDER", f"CHR_Asuna_Braid_{side}", loc=(ox+sx2,-0.08,1.35),
...
# 发箍
B.create("TORUS", "CHR_Asuna_Headband", loc=(ox,0,1.82), scale=(1.0,0.9,0.3),
...
for side, x in [('L',0.32),('R',-0.32)]:
    B.create("UV_SPHERE", f"CHR_Asuna_Shoulder_{side}", loc=(ox+x,0,1.28),
             scale=(1.0,0.7,0.5), mesh_params={"segments":10,"ring_count":8,"radius":0.06})
    B.mat_assign(f"CHR_Asuna_Shoulder_{side}", "SAO_Asuna_Red")

# 红裙
B.create("CONE", "CHR_Asuna_SkirtOuter", loc=(ox,0,0.72),
         mesh_params={"vertices":16,"radius1":0.28,"radius2":0.16,"depth":0.25})
B.mat_assign("CHR_Asuna_SkirtOuter", "SAO_Asuna_Red")

# 白裙
B.create("CONE", "CHR_Asuna_SkirtInner", loc=(ox,0,0.73),
         mesh_params={"vertices":16,"radius1":0.26,"radius2":0.15,"depth":0.22})
B.mat_assign("CHR_Asuna_SkirtInner", "SAO_Fabric_White")

# 腰带
B.create("CYLINDER", "CHR_Asuna_Belt", loc=(ox,0,0.86), scale=(1.0,0.65,1.0),
...
for i,(x,y,z,ln,w,rx,rz) in enumerate(spikes):
    B.create("CONE", f"CHR_Klein_Spike_{i}", loc=(kx+x,y,z),
...
# 头带
B.create("TORUS", "CHR_Klein_Headband", loc=(kx,0,1.80), scale=(1.0,0.9,0.3),
...
# 腰带
B.create("CYLINDER", "CHR_Klein_Belt", loc=(kx,0,0.88), scale=(1.0,0.6,1.0),
         mesh_params={"vertices":16,"radius":0.19,"depth":0.06})
B.mat_assign("CHR_Klein_Belt", "SAO_Leather_Brown")

# 武士刀
B.create("CUBE", "WPN_Katana_Blade", loc=(kx-0.25,-0.10,0.95),
         scale=(0.012,0.004,0.40), rot=(0,0,math.radians(-5)))
B.mat_assign("WPN_Katana_Blade", "SAO_Metal_Silver")

B.create("CYLINDER", "WPN_Katana_Tsuba", loc=(kx-0.25,-0.10,0.55),
         mesh_params={"vertices":16,"radius":0.025,"depth":0.005})
B.mat_assign("WPN_Katana_Tsuba", "SAO_Metal_Dark")

B.create("CYLINDER", "WPN_Katana_Handle", loc=(kx-0.25,-0.10,0.47),

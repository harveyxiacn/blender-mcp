...
def setup_obj(obj, mat_name, col_name='Characters', subsurf=2, outline=0.003):
...
def make_eye(name, loc, radius=0.038, mat='M_EyeBlack'):
    ...
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=16, ring_count=12, radius=radius, location=loc)
...
def make_highlight(name, loc, radius=0.012):
    """眼睛高光"""
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=8, ring_count=6, radius=radius, location=loc)
...
for s, mx in [('L', 1), ('R', -1)]:
    ...
    for fi in range(5):
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=8, radius=0.006, depth=0.035,
            location=(fx, fy, 0.685))
...
for s, x in [('L', 0.18), ('R', -0.18)]:
    bpy.ops.mesh.primitive_uv_sphere_add(segments=10, ring_count=8, radius=0.012, location=(x, -0.07, 1.32))
...
for i in range(8):
    ...
    bpy.ops.mesh.primitive_torus_add(
        major_radius=0.013, minor_radius=0.002,
        location=(sx, -0.10, z))
...
# 柄头
bpy.ops.mesh.primitive_uv_sphere_add(segments=10, ring_count=8, radius=0.012,
    location=(sx, -0.10, 0.37))
...
# 发箍
bpy.ops.mesh.primitive_torus_add(
    major_radius=0.12, minor_radius=0.008,
    location=(ax, 0, 1.72))
...
# 护手横杆
bpy.ops.mesh.primitive_cylinder_add(
    vertices=8, radius=0.003, depth=0.06,
    location=(lx, -0.08, 0.67))
...
# 头带
bpy.ops.mesh.primitive_torus_add(
    major_radius=0.135, minor_radius=0.010,
    location=(kx, 0, 1.74))
...
# 刀镡 (圆形)
bpy.ops.mesh.primitive_cylinder_add(
    vertices=16, radius=0.022, depth=0.004,
    location=(ksx, -0.08, 0.51))
...
print(f"\n{'='*60}")
print(f"  SAO AAA级模型构建完成!")
print(f"{'='*60}")
print(f"  对象总数: {obj_count}")
print(f"  材质总数: {mat_count}")
print(f"  ✓ AAA多层赛璐璐材质 (Diffuse→Toon + Fresnel轮廓光)")
print(f"  ✓ 桐人 (截面放样躯干+头发条带+风衣+描边)")
print(f"  ✓ 逐暗者 (6角截面剑身+装饰纹路握柄)")
print(f"  ✓ 亚丝娜 (身体+长发辫+KoB白红战斗装)")
print(f"  ✓ 闪光 (细剑+护手篮)")
print(f"  ✓ 克莱因 (壮硕体型+刺猬发+武士装+武士刀)")
print(f"  ✓ SubSurf Lv2 + Solidify描边")
print(f"{'='*60}")

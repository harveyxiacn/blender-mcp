...
def link_to_collection(obj, col_name) -> None:
...
def assign_material(obj, mat_name) -> None:
...
def phase1_setup() -> None:
    ...
    lights = [
        ...
        (
            ...
            "SUN",
            (-5, -3, 5),
            (math.radians(60), math.radians(-30), math.radians(-20)),
            1.0,
            (0.85, 0.9, 1.0),
        ),
        (
            ...
            "SUN",
            (0, 5, 8),
            (math.radians(-30), 0, math.radians(180)),
            1.5,
            (1.0, 1.0, 1.0),
        ),
    ]
...
def create_toon_mat(name, base_rgb, shadow_mult=(0.6, 0.55, 0.65), ramp_pos=0.45):
    ...
    shadow = (
        base_rgb[0] * shadow_mult[0],
        base_rgb[1] * shadow_mult[1],
        base_rgb[2] * shadow_mult[2],
        1.0,
    )
...
def phase3_kirito() -> None:
    ...
    obj = bpy.context.active_object
    obj.name = "CHR_Kirito_Torso"
    obj.scale = (0.35, 0.2, 0.45)
    bpy.ops.object.transform_apply(scale=True)
    add_subsurface(obj, 2)
    smooth_shade(obj)
    assign_material(obj, "SAO_Kirito_Black")
    link_to_collection(obj, C)
    add_outline(obj)

    # 头
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=16, ring_count=12, radius=0.22, location=(0, 0, 1.72)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Kirito_Head"
    obj.scale = (1.0, 0.9, 1.05)
    bpy.ops.object.transform_apply(scale=True)
    smooth_shade(obj)
    assign_material(obj, "SAO_Skin")
    link_to_collection(obj, C)
    add_outline(obj)
    ...
    for side, x in [("L", 0.08), ("R", -0.08)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=12, ring_count=8, radius=0.045, location=(x, -0.18, 1.74)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Eye_{side}"
        smooth_shade(obj)
        assign_material(obj, "SAO_Eye_Black")
        link_to_collection(obj, C)
        # 高光
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=8, ring_count=6, radius=0.015, location=(x + 0.02, -0.22, 1.76)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_EyeHL_{side}"
        smooth_shade(obj)
        assign_material(obj, "SAO_Eye_Highlight")
        link_to_collection(obj, C)

    # 手臂
    for side, x in [("L", 0.45), ("R", -0.45)]:
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=12, radius=0.08, depth=0.55, location=(x, 0, 1.1)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Arm_{side}"
        add_subsurface(obj, 1)
        smooth_shade(obj)
        assign_material(obj, "SAO_Kirito_Black")
        link_to_collection(obj, C)
        add_outline(obj)
    ...
    for side, x in [("L", 0.45), ("R", -0.45)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=10, ring_count=8, radius=0.06, location=(x, 0, 0.78)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Hand_{side}"
        obj.scale = (1.0, 0.7, 1.2)
        bpy.ops.object.transform_apply(scale=True)
        smooth_shade(obj)
        assign_material(obj, "SAO_Skin")
        link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.13), ("R", -0.13)]:
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=12, radius=0.09, depth=0.65, location=(x, 0, 0.33)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Leg_{side}"
        add_subsurface(obj, 1)
        smooth_shade(obj)
        assign_material(obj, "SAO_Kirito_Black")
        link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.13), ("R", -0.13)]:
        ...
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Boot_{side}"
    ...
    # 基础球
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=16, ring_count=12, radius=0.25, location=(0, 0.02, 1.76)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Kirito_HairBase"
    obj.scale = (1.05, 1.0, 1.0)
    bpy.ops.object.transform_apply(scale=True)
    smooth_shade(obj)
    assign_material(obj, "SAO_Kirito_Hair")
    link_to_collection(obj, C)
    add_outline(obj)
    ...
    for i, (x, y, z, ln, w, rot) in enumerate(bangs):
        bpy.ops.mesh.primitive_cone_add(
            vertices=8, radius1=w, radius2=0.005, depth=ln, location=(x, y, z)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Bang_{i}"
    ...
    for mult in [1, -1]:
        ...
        for i, (x, y, z, ln, w, rz, rx) in enumerate(sides):
            bpy.ops.mesh.primitive_cone_add(
                vertices=8, radius1=w, radius2=0.008, depth=ln, location=(x * mult, y, z)
            )
            obj = bpy.context.active_object
            obj.name = f"CHR_Kirito_Side_{sn}_{i}"
    ...
    for i, (x, y, z, ln, w, rz) in enumerate(backs):
        bpy.ops.mesh.primitive_cone_add(
            vertices=8, radius1=w, radius2=0.01, depth=ln, location=(x, y, z)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Back_{i}"
    ...
    vb = [
        bm.verts.new(v)
        for v in [(-0.38, 0.12, 1.35), (0.38, 0.12, 1.35), (0.50, 0.13, 0.40), (-0.50, 0.13, 0.40)]
    ]
    ...
    mod2.offset = -1
    smooth_shade(coat)
    assign_material(coat, "SAO_Kirito_Black")
    link_to_collection(coat, C)
    add_outline(coat)

    # 立领
    bpy.ops.mesh.primitive_cylinder_add(vertices=16, radius=0.12, depth=0.08, location=(0, 0, 1.48))
    obj = bpy.context.active_object
    obj.name = "CHR_Kirito_Collar"
    ...
    obj = bpy.context.active_object
    obj.name = "CHR_Kirito_Belt"
    obj.scale = (1.0, 0.6, 1.0)
    bpy.ops.object.transform_apply(scale=True)
    smooth_shade(obj)
    assign_material(obj, "SAO_Leather_Brown")
    link_to_collection(obj, C)

    # 肩扣
    for side, x in [("L", 0.30), ("R", -0.30)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=8, ring_count=6, radius=0.025, location=(x, -0.10, 1.32)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Kirito_Buckle_{side}"
        smooth_shade(obj)
        assign_material(obj, "SAO_Kirito_Silver")
        link_to_collection(obj, C)
...
def phase4_elucidator() -> None:
    ...
    for v in bm.verts:
        if abs(v.co.z - mz) < 0.01:
            v.co.x *= 0.1
            v.co.y *= 0.3
    bmesh.update_edit_mesh(blade.data)
    bpy.ops.object.mode_set(mode="OBJECT")
    add_subsurface(blade, 1)
    smooth_shade(blade)
    assign_material(blade, "SAO_Elucidator_Blade")
    link_to_collection(blade, W)

    # 血槽
    bpy.ops.mesh.primitive_cube_add(size=1, location=(sx, -0.15, 0.97))
    obj = bpy.context.active_object
    obj.name = "WPN_Elucidator_Fuller"
    obj.scale = (0.005, 0.002, 0.32)
    bpy.ops.object.transform_apply(scale=True)
    smooth_shade(obj)
    assign_material(obj, "SAO_Metal_Dark")
    link_to_collection(obj, W)

    # 护手
    bpy.ops.mesh.primitive_cube_add(size=1, location=(sx, -0.15, 0.53))
    obj = bpy.context.active_object
    obj.name = "WPN_Elucidator_Guard"
    obj.scale = (0.06, 0.015, 0.012)
    bpy.ops.object.transform_apply(scale=True)
    add_subsurface(obj, 1)
    smooth_shade(obj)
    assign_material(obj, "SAO_Metal_Dark")
    ...
    for dx in [0.06, -0.06]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=8, ring_count=6, radius=0.012, location=(sx + dx, -0.15, 0.53)
        )
        obj = bpy.context.active_object
        obj.name = f'WPN_Eluc_GuardBall_{"L" if dx>0 else "R"}'
        smooth_shade(obj)
        assign_material(obj, "SAO_Metal_Dark")
        link_to_collection(obj, W)
    ...
    # 握柄
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=10, radius=0.012, depth=0.12, location=(sx, -0.15, 0.46)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_Elucidator_Grip"
    smooth_shade(obj)
    assign_material(obj, "SAO_Leather_Brown")
    link_to_collection(obj, W)
    ...
    # 柄头
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=8, ring_count=6, radius=0.015, location=(sx, -0.15, 0.39)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_Elucidator_Pommel"
    smooth_shade(obj)
    assign_material(obj, "SAO_Metal_Dark")
    link_to_collection(obj, W)
...
def phase5_asuna() -> None:
    ...
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_Torso"
    obj.scale = (0.30, 0.18, 0.40)
    bpy.ops.object.transform_apply(scale=True)
    add_subsurface(obj, 2)
    smooth_shade(obj)
    assign_material(obj, "SAO_Asuna_White")
    link_to_collection(obj, C)
    add_outline(obj)

    # 头
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=16, ring_count=12, radius=0.20, location=(ox, 0, 1.68)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_Head"
    ...
    # 颈
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=10, radius=0.055, depth=0.10, location=(ox, 0, 1.50)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_Neck"
    smooth_shade(obj)
    assign_material(obj, "SAO_Skin")
    link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.07), ("R", -0.07)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=12, ring_count=8, radius=0.045, location=(ox + x, -0.16, 1.70)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Eye_{side}"
        smooth_shade(obj)
        assign_material(obj, "SAO_Asuna_Eye")
        link_to_collection(obj, C)
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=8, ring_count=6, radius=0.015, location=(ox + x + 0.02, -0.20, 1.72)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_EyeHL_{side}"
        smooth_shade(obj)
        assign_material(obj, "SAO_Eye_Highlight")
        link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.40), ("R", -0.40)]:
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=12, radius=0.065, depth=0.50, location=(ox + x, 0, 1.05)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Arm_{side}"
        add_subsurface(obj, 1)
        smooth_shade(obj)
        assign_material(obj, "SAO_Fabric_White")
        link_to_collection(obj, C)
        add_outline(obj)
    ...
    for side, x in [("L", 0.40), ("R", -0.40)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=10, ring_count=8, radius=0.05, location=(ox + x, 0, 0.75)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Hand_{side}"
        obj.scale = (1.0, 0.7, 1.2)
        bpy.ops.object.transform_apply(scale=True)
        smooth_shade(obj)
        assign_material(obj, "SAO_Skin")
    ...
    for side, x in [("L", 0.11), ("R", -0.11)]:
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=12, radius=0.075, depth=0.55, location=(ox + x, 0, 0.35)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Leg_{side}"
        add_subsurface(obj, 1)
        smooth_shade(obj)
        assign_material(obj, "SAO_Skin")
        link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.11), ("R", -0.11)]:
        ...
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Boot_{side}"
    ...
    # === 头发 ===
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=16, ring_count=12, radius=0.23, location=(ox, 0.02, 1.72)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_HairBase"
    smooth_shade(obj)
    assign_material(obj, "SAO_Asuna_Hair")
    link_to_collection(obj, C)
    add_outline(obj)
    ...
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_LongHair"
    obj.scale = (0.20, 0.04, 0.55)
    bpy.ops.object.transform_apply(scale=True)
    bpy.ops.object.mode_set(mode="EDIT")
    ...
    for i, (x, y, z, ln, w, rot) in enumerate(a_bangs):
        bpy.ops.mesh.primitive_cone_add(
            vertices=8, radius1=w, radius2=0.005, depth=ln, location=(ox + x, y, z)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Bang_{i}"
        obj.rotation_euler = (math.radians(70), 0, math.radians(rot))
        smooth_shade(obj)
        assign_material(obj, "SAO_Asuna_Hair")
        link_to_collection(obj, C)
    ...
    for side, sx in [("L", 0.15), ("R", -0.15)]:
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=8, radius=0.025, depth=0.50, location=(ox + sx, -0.08, 1.35)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Braid_{side}"
    ...
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_ChestArmor"
    obj.scale = (0.32, 0.10, 0.25)
    bpy.ops.object.transform_apply(scale=True)
    add_subsurface(obj, 2)
    smooth_shade(obj)
    assign_material(obj, "SAO_Asuna_White")
    ...
    for dim, nm in [((0.08, 0.005, 0.02), "H"), ((0.02, 0.005, 0.08), "V")]:
        ...
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Cross_{nm}"
        obj.scale = dim
        bpy.ops.object.transform_apply(scale=True)
        assign_material(obj, "SAO_Asuna_Red")
        link_to_collection(obj, C)

    # 肩甲
    for side, x in [("L", 0.32), ("R", -0.32)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=10, ring_count=8, radius=0.06, location=(ox + x, 0, 1.28)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Asuna_Shoulder_{side}"
        obj.scale = (1.0, 0.7, 0.5)
        bpy.ops.object.transform_apply(scale=True)
        smooth_shade(obj)
        assign_material(obj, "SAO_Asuna_Red")
        link_to_collection(obj, C)
    ...
    # 裙子
    bpy.ops.mesh.primitive_cone_add(
        vertices=16, radius1=0.28, radius2=0.16, depth=0.25, location=(ox, 0, 0.72)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_SkirtOuter"
    smooth_shade(obj)
    assign_material(obj, "SAO_Asuna_Red")
    link_to_collection(obj, C)

    bpy.ops.mesh.primitive_cone_add(
        vertices=16, radius1=0.26, radius2=0.15, depth=0.22, location=(ox, 0, 0.73)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_SkirtInner"
    smooth_shade(obj)
    assign_material(obj, "SAO_Fabric_White")
    link_to_collection(obj, C)
    ...
    # 腰带
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=16, radius=0.16, depth=0.04, location=(ox, 0, 0.86)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Asuna_Belt"
    obj.scale = (1.0, 0.65, 1.0)
    bpy.ops.object.transform_apply(scale=True)
    smooth_shade(obj)
    assign_material(obj, "SAO_Asuna_White")
    link_to_collection(obj, C)
...
def phase6_lambent_light() -> None:
    ...
    # 剑身
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=8, radius=0.005, depth=0.75, location=(ox, -0.12, 1.10)
    )
    blade = bpy.context.active_object
    ...
    bpy.ops.object.mode_set(mode="OBJECT")
    smooth_shade(blade)
    assign_material(blade, "SAO_LambentLight_Blade")
    link_to_collection(blade, W)

    # 护手环
    bpy.ops.mesh.primitive_torus_add(
        major_radius=0.04, minor_radius=0.005, location=(ox, -0.12, 0.72)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_LL_GuardRing"
    obj.rotation_euler = (math.radians(90), 0, 0)
    smooth_shade(obj)
    assign_material(obj, "SAO_Metal_Silver")
    link_to_collection(obj, W)

    # 护手横条
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=8, radius=0.004, depth=0.08, location=(ox, -0.12, 0.72)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_LL_GuardBar"
    ...
    # 握柄
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=8, radius=0.010, depth=0.10, location=(ox, -0.12, 0.66)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_LL_Grip"
    smooth_shade(obj)
    assign_material(obj, "SAO_Fabric_White")
    link_to_collection(obj, W)
    ...
    # 柄头
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=8, ring_count=6, radius=0.012, location=(ox, -0.12, 0.60)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_LL_Pommel"
    smooth_shade(obj)
    assign_material(obj, "SAO_Metal_Silver")
    link_to_collection(obj, W)
...
def phase7_klein() -> None:
    ...
    obj = bpy.context.active_object
    obj.name = "CHR_Klein_Torso"
    obj.scale = (0.38, 0.22, 0.45)
    bpy.ops.object.transform_apply(scale=True)
    add_subsurface(obj, 2)
    smooth_shade(obj)
    assign_material(obj, "SAO_Klein_Red")
    link_to_collection(obj, C)
    add_outline(obj)

    # 头
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=16, ring_count=12, radius=0.22, location=(ox, 0, 1.72)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Klein_Head"
    obj.scale = (1.0, 0.92, 1.0)
    bpy.ops.object.transform_apply(scale=True)
    smooth_shade(obj)
    assign_material(obj, "SAO_Skin")
    link_to_collection(obj, C)
    ...
    # 颈
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=10, radius=0.07, depth=0.12, location=(ox, 0, 1.53)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Klein_Neck"
    smooth_shade(obj)
    assign_material(obj, "SAO_Skin")
    link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.08), ("R", -0.08)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=12, ring_count=8, radius=0.035, location=(ox + x, -0.18, 1.74)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Klein_Eye_{side}"
        obj.scale = (1.2, 1.0, 0.7)
        bpy.ops.object.transform_apply(scale=True)
        smooth_shade(obj)
        assign_material(obj, "SAO_Eye_Black")
        link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.48), ("R", -0.48)]:
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=12, radius=0.09, depth=0.55, location=(ox + x, 0, 1.1)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Klein_Arm_{side}"
        add_subsurface(obj, 1)
        smooth_shade(obj)
        assign_material(obj, "SAO_Klein_Red")
        link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.48), ("R", -0.48)]:
        bpy.ops.mesh.primitive_uv_sphere_add(
            segments=10, ring_count=8, radius=0.065, location=(ox + x, 0, 0.78)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Klein_Hand_{side}"
        smooth_shade(obj)
        assign_material(obj, "SAO_Skin")
        link_to_collection(obj, C)
    ...
    for side, x in [("L", 0.14), ("R", -0.14)]:
        bpy.ops.mesh.primitive_cylinder_add(
            vertices=12, radius=0.11, depth=0.65, location=(ox + x, 0, 0.33)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Klein_Leg_{side}"
        add_subsurface(obj, 1)
        smooth_shade(obj)
        assign_material(obj, "SAO_Klein_Cream")
        link_to_collection(obj, C)
        add_outline(obj)
    ...
    for side, x in [("L", 0.14), ("R", -0.14)]:
        ...
        obj = bpy.context.active_object
        obj.name = f"CHR_Klein_Sandal_{side}"
        obj.scale = (0.10, 0.14, 0.04)
        bpy.ops.object.transform_apply(scale=True)
        smooth_shade(obj)
        assign_material(obj, "SAO_Leather_Brown")
        link_to_collection(obj, C)
    ...
    # === 头发(刺猬式) ===
    bpy.ops.mesh.primitive_uv_sphere_add(
        segments=14, ring_count=10, radius=0.24, location=(ox, 0.02, 1.78)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Klein_HairBase"
    smooth_shade(obj)
    assign_material(obj, "SAO_Klein_Hair")
    link_to_collection(obj, C)
    add_outline(obj)
    ...
    for i, (x, y, z, ln, w, rx, rz) in enumerate(spikes):
        bpy.ops.mesh.primitive_cone_add(
            vertices=6, radius1=w, radius2=0.005, depth=ln, location=(ox + x, y, z)
        )
        obj = bpy.context.active_object
        obj.name = f"CHR_Klein_Spike_{i}"
    ...
    bpy.ops.mesh.primitive_cube_add(size=1, location=(ox, -0.05, 1.12))
    obj = bpy.context.active_object
    obj.name = "CHR_Klein_ChestArmor"
    obj.scale = (0.34, 0.08, 0.22)
    bpy.ops.object.transform_apply(scale=True)
    add_subsurface(obj, 1)
    smooth_shade(obj)
    assign_material(obj, "SAO_Klein_Brown")
    link_to_collection(obj, C)

    # 腰带
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=16, radius=0.19, depth=0.06, location=(ox, 0, 0.88)
    )
    obj = bpy.context.active_object
    obj.name = "CHR_Klein_Belt"
    obj.scale = (1.0, 0.6, 1.0)
    bpy.ops.object.transform_apply(scale=True)
    smooth_shade(obj)
    assign_material(obj, "SAO_Leather_Brown")
    ...
    blade = bpy.context.active_object
    blade.name = "WPN_Klein_Katana_Blade"
    blade.scale = (0.012, 0.004, 0.40)
    bpy.ops.object.transform_apply(scale=True)
    blade.rotation_euler = (0, 0, math.radians(-5))
    add_subsurface(blade, 1)
    smooth_shade(blade)
    assign_material(blade, "SAO_Metal_Silver")
    link_to_collection(blade, W)

    # 刀镡
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=16, radius=0.025, depth=0.005, location=(ox - 0.25, -0.10, 0.55)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_Klein_Katana_Tsuba"
    smooth_shade(obj)
    assign_material(obj, "SAO_Metal_Dark")
    link_to_collection(obj, W)
    ...
    # 柄
    bpy.ops.mesh.primitive_cylinder_add(
        vertices=10, radius=0.012, depth=0.15, location=(ox - 0.25, -0.10, 0.47)
    )
    obj = bpy.context.active_object
    obj.name = "WPN_Klein_Katana_Handle"
    smooth_shade(obj)
    assign_material(obj, "SAO_Klein_Red")
    link_to_collection(obj, W)
